            ...
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                @foreach ($this->getPresets() as $key => $c)
                    <button type="button" wire:click="applyPreset('{{ $key }}')"
                        ...
                        <span class="flex h-9 w-9 shrink-0 overflow-hidden rounded-lg ring-1 ring-black/5">
                            <span class="w-1/3" style="background: {{ $c[0] }}"></span>
                            <span class="w-1/3" style="background: {{ $c[1] }}"></span>
                            <span class="w-1/3" style="background: {{ $c[2] }}"></span>
                        </span>
                    ...
                    <div class="overflow-hidden rounded-xl ring-1 ring-black/5">
                        ...
                        <div class="space-y-4 bg-white p-5 dark:bg-gray-900">
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                <span class="inline-flex items-center gap-2 rounded-lg border-2 px-4 py-2 text-sm font-bold"
                                      style="border-color: {{ $A }}; color: {{ $A }};">
                                    {{ __('theme.preview.btn_accent') }}
                                </span>
                            </div>

                            {{-- card --}}
                            <div class="rounded-xl border p-4" style="border-color: {{ $S }}33;">
                                <div class="flex items-center gap-3">
                                    <span class="flex h-10 w-10 items-center justify-center rounded-lg text-white" style="background: {{ $P }}">
