    ...
    <section class="py-7">
        ...
        <div class="space-y-3">
            @foreach ($items as $software)
                @php($size = (int) ($software->total_size_bytes ?? 0))
                <a href="{{ route('software.show', $software) }}"
                   class="card-luxury group flex items-center gap-3 p-3.5 transition hover:border-saudi-green/40 sm:gap-4">
                    ...
                    @if (! empty($ranked))
                        <span class="hidden h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-saudi-green/10 font-cairo text-base font-black text-saudi-green sm:flex" dir="ltr">{{ $loop->iteration }}</span>
                    @endif
